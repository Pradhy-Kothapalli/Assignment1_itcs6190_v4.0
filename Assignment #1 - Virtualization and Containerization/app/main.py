...
def main():
    ...
    with conn, conn.cursor() as cur:
        ...
        cur.execute("""
select city, avg(fare) as avg_fare from trips group by city order by city;
 """)
        by_city = [{"city": c, "avg_fare": float(a)} 
                   for (c, a) in cur.fetchall()]

        # Top N trips by DEFINE YOUR QUERY
        cur.execute("""
 select city, minutes, fare from trips order by minutes desc, id asc limit %s;
 """, (TOP_N,))
        top = [{"city": c, "minutes": int(m), "fare": float(f)} 
